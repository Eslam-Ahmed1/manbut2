 ...
 */
export const validateImageFile = (imageBuffer, mimeType) => {
    // 1. MIME type whitelist
    if (!ALLOWED_MIME_TYPES.has(mimeType)) {
        throw new appError(`Unsupported file type "${mimeType}". Only JPEG, PNG, and WebP images are allowed.`, 400);
    }
    // 2. File size
    if (imageBuffer.length > MAX_FILE_SIZE_BYTES) {
        const sizeMB = (imageBuffer.length / (1024 * 1024)).toFixed(1);
        throw new appError(`File too large (${sizeMB} MB). Maximum allowed size is ${MAX_FILE_SIZE_BYTES / (1024 * 1024)} MB.`, 400);
    }
    ...
    const matchesMagic = MAGIC_BYTES.some(({ bytes }) => bytes.every((byte, i) => imageBuffer[i] === byte));
    if (!matchesMagic) {
        throw new appError("File content does not match a valid image format. The file may be corrupted or renamed.", 400);
    }
};
 ...
 */
export const verifyPlantImage = async (imageBuffer, mimeType, confidenceThreshold) => {
    ...
    try {
        ...
        logger.debug(`   ${parsed.isPlant ? "✅" : "❌"} [VALIDATE] Plant check: isPlant=${parsed.isPlant}, confidence=${parsed.confidence} — ${Date.now() - t0}ms`);
        // Apply confidence threshold
        if (!parsed.isPlant || parsed.confidence < confidenceThreshold) {
            throw new appError(`This does not appear to be a plant image. Reason: ${parsed.reason || "Not a plant"}`, 422);
        }
        return parsed;
    }
    catch (error) {
 ...
 */
export const validateAndVerifyImage = async (imageBuffer, mimeType) => {
