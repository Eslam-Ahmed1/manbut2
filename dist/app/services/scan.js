...
const analyzePlantImage = async (userId, imageBuffer, mimeType) => {
    ...
    try {
        ...
        if (detectedDiseases.length === 0) {
            ...
            logger.debug(`   🦠 [SCAN] Detected ${detectedDiseases.length} disease(s) via ${meta.source}: ${detectedDiseases.map((d) => d.name).join(", ")}`);
        }
        const diseaseIds = [];
        ...
        logger.info(`✅ [SCAN] Complete — user: ${userId}, ${detectedDiseases.length} disease(s), source: ${meta.source} — ${Date.now() - startTime}ms`);
        return {
